...
class _MainDecisionClient:
        ...
        *,
        ...
        return await self._run_stage(
            ...
            validate=lambda value: validate_draft_binding(
                value, run_id=run_id, snapshot_id=snapshot_id, language=language,
            ),
        )
    ...
    async def _run_stage(
        ...
        *,
        ...
        response_schema = contract_type.model_json_schema()
...
class StarOrchestrator:
    ...
    async def execute(self, owner_username: str, run_id: str) -> OrchestrationResult:
        ...
        try:
            evidence = await self._retrieve_and_bind(claimed, owner_username)
    ...
    def _claim_run(self, owner_username: str, run_id: str) -> _ClaimedRun:
        with self._database.connect() as connection:
            ...
            language = select_response_language(str(item["text"]) for item in user_rows)
            connection.execute(
                """
                UPDATE runs
...
def _validate_suggestion_set(
    ...
    *,
    ...
    if forbidden_suggestion_ids.intersection(
        item.suggestion_id for item in suggestions
    ):
        raise ValueError("reviewer reused a suggestion ID from an earlier role")
    for suggestion in suggestions:
        if suggestion.category != expected_category:
            raise ValueError("reviewer returned a suggestion for another role")
        if not suggestion.suggestion_id.startswith(expected_id_prefix):
            raise ValueError("reviewer suggestion ID has the wrong role prefix")
        if not set(suggestion.affected_claim_ids) <= draft_claim_ids:
            raise ValueError("reviewer referenced an unknown draft claim")
        if not set(suggestion.evidence_refs) <= factpack_record_ids:
            raise ValueError("reviewer referenced evidence outside the frozen FactPack")
